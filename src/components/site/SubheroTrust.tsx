...
export function SubheroTrust() {
  return (
    <section
      id="trust"
      className="relative overflow-hidden py-16 md:py-24 bg-[var(--cream)]"
    >
      {/* Background images */}
      <img
        src={farmerDesktop}
      ...
      {/* Scrims using cream token */}
      <div className="md:hidden absolute inset-0 bg-gradient-to-b from-[var(--cream)] via-[color-mix(in_oklab,var(--cream)_85%,transparent)] to-transparent" />
      ...
      <div className="pointer-events-none absolute inset-0">
        <Leaf className="absolute top-10 left-[6%] size-10 text-emerald-700/15 rotate-12 blur-[1px] animate-float" />
        <Leaf className="absolute top-[35%] left-[42%] size-8 text-emerald-700/10 -rotate-45 blur-[1px] animate-float" style={{ animationDelay: "1.5s" }} />
        <Leaf className="absolute bottom-12 left-[10%] size-12 text-emerald-700/15 rotate-45 blur-[1px] animate-float" style={{ animationDelay: "0.8s" }} />
      ...
      <div className="relative max-w-7xl mx-auto px-6 grid md:grid-cols-12 md:gap-8 md:items-center">
        ...
        <div className="md:col-span-7 space-y-6">
          <span className="inline-block bg-emerald-800 text-white font-bold text-xs px-3 py-1.5 rounded-full shadow-sm">
            Trusted By Farmers Across India
          </span>

          <h2 className="text-emerald-950 font-display font-bold text-3xl md:text-5xl leading-tight [text-shadow:0_1px_0_rgba(255,255,255,0.6)]">
          ...
          <p className="text-stone-800 font-medium text-base md:text-lg max-w-xl leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {values.map((v) => (
              <div key={v.title} className="flex items-center gap-3">
                <div className="size-11 rounded-full border-2 border-emerald-700/30 grid place-items-center text-emerald-700 bg-white/60 backdrop-blur-sm shrink-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-stone-600 text-xs">{v.meta}</div>
                </div>
              </div>
            ))}
          </div>
